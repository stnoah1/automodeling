function [gridOUTPUT,varargout] = VOXELISE(gridX,gridY,gridZ,theta,varargin)
...
if narginremaining==5
  ...
  elseif ischar(varargin{1})
    ...
    T_1=[cos(theta) sin(theta) 0;-sin(theta) cos(theta) 0; 0 0 1];
    n=length(meshXYZ);
    for i=1:n
        meshXYZ(i,:,:)=T_1*squeeze(meshXYZ(i,:,:));
    end
  else
    meshXYZ = varargin{1};
  end
elseif narginremaining==7
